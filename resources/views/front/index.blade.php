		...
		<nav id="Category" class="max-w-[1130px] mx-auto flex justify-center items-center gap-4 mt-[30px]">
            ...
            <a href="{{ route('front.category', $category->slug) }}" class="rounded-full p-[12px_22px] flex gap-[10px] font-semibold transition-all duration-300 border border-[#EEF0F7] hover:ring-2 hover:ring-[#FF6B18]">
				<div class="flex w-6 h-6 shrink-0">
					<img src="{{ route('private.file', $category->icon) }}" alt="icon" class=""/>
				</div>
...
@if(isset($bannerads) && $bannerads)  {{-- Baris ini memastikan variabel ada dan bukan null --}}
...
{{-- PERBAIKAN: Bungkus seluruh blok dengan pengecekan utama untuk Featured Article --}}
@if (isset($entertainment_featured_articles) && $entertainment_featured_articles)

    <div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
        
        {{-- PERBAIKAN BANNER ADS: Cek apakah $bannerads ada sebelum mengakses propertinya --}}
        @if (isset($bannerads) && $bannerads)
            <img src="{{ route('private.file', $bannerads->thumbnail) }}" class="absolute object-cover w-full h-full thumbnail" alt="icon" />
        @else
            {{-- Opsional: Tambahkan placeholder jika banner ads tidak ada --}}
            <img src="/path/to/default-placeholder.jpg" class="absolute object-cover w-full h-full thumbnail" alt="default icon" />
        @endif
        ...
        <div class="card-detail w-full flex items-end p-[30px] relative z-20">
            <div class="flex flex-col gap-[10px]">
                ...
                <a href="details.html" class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">
                    {{-- Akses properti aman karena sudah dicek di blok @if atas --}}
...
@endif {{-- Akhir dari pengecekan kondisi $entertainment_featured_articles --}}
